...
void Process_CMD2(void)
{
	
}


void Process_CMD5(void)
{
  unsigned char i;
...
void Modbus_NewCMD(void)
{
unsigned long int temp32;	
	
  if( Coldw.CMD          == 0 )    //命令寄存器	
	    {
	    	return;
	    }
	...
	switch( temp32 )
       ...
       case 1:
        ...
        case 2:    //开启老化，启动老化流程1，这是常用开启老化命令
            Process_CMD2();
          break;


          case 5:  //结束老化，结束老化流程1，这是常用结束老化命令：(关闭VD) 
           Process_CMD5();
          break;
	...
	switch (*(p+1))                   //功能码
	   {
		  ...
		  ModbusCommand1();  //收到参数时执行一下,DOWNLOAD  INIT
